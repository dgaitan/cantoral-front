...
- Name: auth-flow.spec.ts >> Login page >> shows email and password fields with Ingresar button
- Location: tests/e2e/auth-flow.spec.ts:120:7
...
```yaml
- link "Cancionero Católico":
  - /url: /
  - img "Cancionero Católico"
- img "Cancionero Católico"
...
```ts
  22  | async function mockLoginError(page: Page) {
  23  |   await page.route(`${BACKEND}/auth/login`, (route) =>
  24  |     route.fulfill({
  25  |       status: 400,
